...
from backend import signup_handler, database

db_conn = database.Database()
handler = signup_handler.SignupHandler(db_conn) # Instantiate the class and pass in the db_conn object
...
class SignupScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Button(self, text="CREATE ACCOUNT", bg="#C8541A", fg="#b8a99a",
                font=("Courier", 9), relief="flat", width=20,
                command=self.handle_signup).pack(ipady=6)
        
        tk.Button(self, text="LOGIN", bg="#1a0a00", fg="#f5ead8",
                font=("Courier", 10, "bold"), relief="flat", width=20,
                command=lambda: controller.show_frame("LoginScreen")).pack(pady=(12, 6), ipady=6)
...
if __name__ == "__main__":
    ...
    app = SignupScreen(root, controller=None)
